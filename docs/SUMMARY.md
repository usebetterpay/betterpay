...
| 3 | **[generic billing SDKs-feature-mapping.md](generic billing SDKs-feature-mapping.md)** | ~420 | 182 generic billing SDKs features mapped to BetterPay |
| 4 | **[provider-research-2026.md](provider-research-2026.md)** | ~430 | Provider pricing, APIs, BI regulations, QRIS data |
| 5 | **[COMPARISON.md](../COMPARISON.md)** | ~590 |  vs BetterPay architecture comparison |
| | **Total** | **~2,960** | |
...
│  ├── 6 provider adapters (Midtrans, Xendit, Duitku, Pakasir, Tripay, Mayar) │
...
| # | Question | Answer | Why |
|---|----------|--------|-----|
| 1 | Framework or Service? | **Framework** (embedded library) | /billing domain model, user owns data |
...
| 6 | How to handle migrations? | **Auto dev, block prod, CLI push** | generic billing SDKs-proven pattern |
| 7 | How to handle notifications? | **Plugin-based** (core fires events) | Separation of concerns |
| 8 | Client SDK design? | **Proxy with type inference** | billing domain model, zero config |
...
Your App (Any Framework)
  Next.js │ Hono │ Express │ Fastify │ Bun │ Cloudflare
         ...
         │          │          │          │
    ┌────┴───┐ ┌───┴────┐ ┌──┴─────┐ ┌─┴───────┐
    │Midtrans│ │ Xendit │ │ Duitku │ │ Pakasir │  ← Extracted 
    └────────┘ └────────┘ └────────┘ └─────────┘
```
...
@betterpay/midtrans          ← Provider adapter ()
@betterpay/xendit            ← Provider adapter ()
@betterpay/duitku            ← Provider adapter ()
@betterpay/pakasir           ← Provider adapter ()
...
Week 2: Extract Midtrans + Xendit adapters 
...
1. **Deep dive generic billing SDKs** (sample/generic billing SDKs) — read all source files, understand every pattern
2. **Deep dive ** (sample/) — plugin system, adapter factory, hooks, better-call
3. **Comparison** — head-to-head architecture comparison, identified gaps
4. **Feature mapping** — 182 generic billing SDKs features cataloged and mapped
5. **Provider research** —  codebase analysis (4 existing provider adapters)
...
|--------|-----------------|
| generic billing SDKs v0.0.6 source | Domain model, plan DSL, entitlement engine, webhook pipeline |
|  v1.6.15 source | Plugin system, adapter factory, better-call, hooks, client SDK |
| production payment gateway | 6 provider adapters, circuit breaker, reconciliation, state machine |
|  billing system | Subscription lifecycle, invoicing, cron jobs, email templates |
