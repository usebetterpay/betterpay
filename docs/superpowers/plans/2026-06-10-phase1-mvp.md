...
**Architecture:** Monorepo with plugin-first architecture. Core package provides better-call router, plugin system, provider interface with priority-based selection and circuit breaker. Provider adapters  production code. Webhook pipeline with idempotency and signature verification.
...
|------|-----------|--------|
| 6 | Circuit Breaker (extract ) | ⬜ |
| 7 | Transaction Schema + Service | ⬜ |
| 8 | Webhook Handler + Idempotency | ⬜ |
| 9 | Midtrans Adapter (extract ) | ⬜ |
| 10 | Xendit Adapter (extract ) | ⬜ |
