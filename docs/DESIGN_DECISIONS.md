...
| 8 | [Client SDK](#8-client-sdk) | Proxy with type inference (generic billing SDKs style) | 2026-06-10 |
...
| 10 | [Build Strategy](#10-build-strategy) | Extract providers , rewrite everything else | 2026-06-10 |
...
**Konteks:** BetterPay bisa jadi framework (kayak ) atau standalone service (kayak Stripe).
...
**Alasan:**
1.  dan generic billing SDKs pakai model yang sama — proven
...
**Alasan:**
1. Circuit breaker sudah ada di  code — tinggal leverage
...
**Alasan:**
1. generic billing SDKs already proves this pattern works
...
**Alasan:**
1. plugin-first architecture — core fires events, plugins handle side effects
...
**Keputusan:** Proxy-based with type inference from server instance (generic billing SDKs style).

**Opsi yang dipertimbangkan:**
- A) Dynamic proxy (generic billing SDKs style) — `payClient.subscribe()` auto-maps to endpoint
- B) Explicit typed methods ( style) — requires importing plugins
- C) Proxy + explicit hybrid

**Keputusan:** Option A.

**Alasan:**
1. generic billing SDKs's proxy approach works — ~40 lines, zero dependencies
...
**Alasan:**
1. This is EXACTLY what 's `payment_transaction` table does — production-proven
...
**Keputusan:** Extract provider adapters , rewrite everything else.

**Konteks:** Ada production code di `payment gateway core` () dengan 4 working provider adapters.

**Opsi yang dipertimbangkan:**
- A) Extract & wrap — take  code as-is, restructure
- B) Rewrite from scratch — ignore  code
...
❌ Webhook handler → generic billing SDKs-style idempotency pipeline
...
2. generic billing SDKs already has this pattern (`testing: { enabled: true }`)
...
2. 's primary use case is one-time payments (campaign management)
...
2. Customer metadata already solves 80% of multi-tenant needs:
   ...
   ```
3. generic billing SDKs is single-merchant.  is single-tenant.
...
@betterpay/midtrans  (extracted )
@betterpay/xendit    (extracted )
...
@betterpay/duitku   (extracted )
@betterpay/pakasir  (extracted )
...
- `/home/ujang/0new/devstack/betterpay/docs/generic billing SDKs-feature-mapping.md` — 182 generic billing SDKs features mapped to BetterPay
- `/home/ujang/0new/devstack/betterpay/COMPARISON.md` —  vs BetterPay architecture comparison
...
*Codebase references: production payment gateway (production), generic billing SDKs v0.0.6,  v1.6.15*
