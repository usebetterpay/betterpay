...
export interface IPay88Config { apiKey?: string; merchantId?: string; partnerId?: string; secretKey?: string; privateKey?: string; publicKey?: string; isSandbox?: boolean; priority?: number; fetch?: typeof globalThis.fetch; }
...
export function ipay88Provider(config: IPay88Config): PaymentProvider & { priority?: number } {
  ...
  return {
    ...
    async createPaymentLink(data: CreatePaymentLinkInput):Promise<PaymentLinkResult>{
      ...
      const result=await req('/api/payment', body);
