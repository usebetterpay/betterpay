...
export interface PaylabsConfig { apiKey?: string; merchantId?: string; partnerId?: string; secretKey?: string; privateKey?: string; publicKey?: string; isSandbox?: boolean; priority?: number; fetch?: typeof globalThis.fetch; }
...
export function paylabsProvider(config: PaylabsConfig): PaymentProvider & { priority?: number } {
  ...
  return {
    ...
    async createPaymentLink(data: CreatePaymentLinkInput):Promise<PaymentLinkResult>{
      ...
      const result=await req('/v1/payment/create', body);
    ...
    async checkStatus(id:string):Promise<StatusResult>{ const d=await req('/v1/payment/status',{orderId:id}); return { providerTransactionId:String(d.payment_id??d.trxId??id), status:mapStatus(d.status??d.paymentStatus??d.responseCode), amount:Number(d.amount??0), currency:'IDR', raw:d }; },
