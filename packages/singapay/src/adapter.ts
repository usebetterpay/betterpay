...
export interface SingaPayConfig { apiKey?: string; merchantId?: string; partnerId?: string; secretKey?: string; privateKey?: string; publicKey?: string; isSandbox?: boolean; priority?: number; fetch?: typeof globalThis.fetch; }
...
export function singapayProvider(config: SingaPayConfig): PaymentProvider & { priority?: number } {
  const baseUrl = config.isSandbox === false ? 'https://api-lb.singapay.id' : 'https://api-lb-sandbox.singapay.id';
  const doFetch = config.fetch ?? fetch;
  async function req(path:string, body:Record<string,unknown>, method='POST'){ const ts=new Date().toISOString(); const serialized=JSON.stringify(body); let sig=''; if(config.secretKey) sig=createSignature(config.partnerId??config.merchantId??'test', ts, serialized, config.secretKey); const headers:Record<string,string>={'Content-Type':'application/json',Accept:'*/*','X-TIMESTAMP':ts,'X-SIGNATURE':sig,'X-PARTNER-ID':config.partnerId??config.merchantId??'test'}; const res=await doFetch(`${baseUrl}${path}`,{method,headers,body:serialized}); const raw=await res.text(); let data:any; try{data=JSON.parse(raw);}catch{throw new Error(`SingaPay request failed: ${res.status} ${raw}`);} if(!res.ok) throw new Error(`SingaPay request failed: ${res.status} ${raw}`); if(data.responseCode && !String(data.responseCode).startsWith('200') && String(data.responseCode)!=='00') throw new Error(`SingaPay request failed: ${res.status} ${data.responseMessage??raw}`); return data; }
  return {
    id:'singapay', name:'SingaPay', priority:config.priority,
    ...
    async createPaymentLink(data: CreatePaymentLinkInput):Promise<PaymentLinkResult>{
      ...
      if(config.secretKey) body.signature=createSignature(config.partnerId??'test', new Date().toISOString(), JSON.stringify(body), config.secretKey);
      const result=await req('/api/v1/va/create', body);
    ...
    async checkStatus(id:string):Promise<StatusResult>{ const d=await req('/api/v1/va/status',{orderId:id}); return { providerTransactionId:String(d.payment_id??d.trxId??id), status:mapStatus(d.status??d.paymentStatus??d.responseCode), amount:Number(d.amount??0), currency:'IDR', raw:d }; },
    async verifyWebhook(data:WebhookData):Promise<boolean>{ const sig=data.headers['x-signature']??data.headers['X-Signature']??data.headers['X-SIGNATURE']; if(!sig || !config.secretKey) return false; try{return verifySignature(config.partnerId??'test', data.headers['x-timestamp']??'', data.body, sig, config.secretKey);}catch{return false;} },
