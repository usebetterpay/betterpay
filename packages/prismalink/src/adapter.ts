...
export interface PrismalinkConfig { apiKey?: string; merchantId?: string; partnerId?: string; secretKey?: string; privateKey?: string; publicKey?: string; isSandbox?: boolean; priority?: number; fetch?: typeof globalThis.fetch; }
...
export function prismalinkProvider(config: PrismalinkConfig): PaymentProvider & { priority?: number } {
  ...
  return {
    ...
    async createPaymentLink(data: CreatePaymentLinkInput):Promise<PaymentLinkResult>{
      ...
      const result=await req('/api/payment/create', body);
    ...
    async checkStatus(id:string):Promise<StatusResult>{ const d=await req('/api/payment/status',{orderId:id}); return { providerTransactionId:String(d.payment_id??d.trxId??id), status:mapStatus(d.status??d.paymentStatus??d.responseCode), amount:Number(d.amount??0), currency:'IDR', raw:d }; },
