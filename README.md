...
| **[COMPARISON.md](COMPARISON.md)** |  vs BetterPay comparison |
...
1. **Architecture** (plugin-first) — Plugin system, better-call router, hooks, adapter factory
2. **Domain model** (subscriptions + entitlements) — Plans, features, entitlements, subscriptions, webhooks
3. **Payment infra** () — Provider adapters, circuit breaker, reconciliation, state machine
...
Your App (Any Framework)
  ...
  ┌─────┼─────────┐
  │     │         │
Midtrans Xendit  Duitku    ← Provider plugins (extracted )
```
